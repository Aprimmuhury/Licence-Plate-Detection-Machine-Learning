"""
Simplified Live License Plate Detection from Mobile Camera Stream
Detects license plates using YOLO (without OCR for now)
"""
...
def main():
    ...
    if not cap.isOpened():
        ...
        print("Note: This script expects a mobile camera stream.")
        print("For testing, you can use a video file instead.")
        sys.exit(1)
    ...
    csv_writer.writerow(["Timestamp", "Confidence", "X1", "Y1", "X2", "Y2"])  # headers
    ...
    try:
        while True:
            ...
            for box in results[0].boxes:
                ...
                label = model.names[cls_id]  # class name (e.g. "objects")
                ...
                csv_writer.writerow([timestamp, confidence, x1, y1, x2, y2])
                csv_file.flush()
                print(f"Detected Plate: {label} | Conf: {confidence:.4f} | Time: {timestamp}")
